...
int E1000_enable(void *base)
{
	...
	registers[TCTL / sizeof(int)] = TCTL_EN + TCTL_PSP + (0x10 << 4)
			+ (0x40 << 12);
...
#define KADDR(name) ((long) name + VAddr)
	...
	{
		((struct rx_desc *) (KADDR((physaddr_t)receive_descriptor)))[i].buffer_addr =
				AllocPage(0);
	}
...
void queue_packet(void *packet, int length)
{
	...
	struct tx_desc *descriptor =
			(struct tx_desc *) (KADDR((physaddr_t)transmit_descriptor));
	if (!(descriptor[nexttxpacket].status & 0x00100000) && checkDD)
		return;
	int i = nexttxpacket++;
...
int receive_packet(void *buffer, int *length)
{
	struct rx_desc *descriptor =
			(struct rx_desc *) (KADDR((physaddr_t)receive_descriptor));
	...
	nextrxpacket++;
	memcpy(buffer,
			(void *) ((physaddr_t *) KADDR(descriptor[i].buffer_addr)), descriptor[i].length);
