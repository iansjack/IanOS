...
long FindFile(char name[13])
{
	...
	char dirname[11];
	if (NameToDirName(name, dirname)) return 0;
	...
	for (n = 0; n < RootDirectoryEntries; n++)
	{
		...
		if (done == 0) return (long) &RootDirectory[n];
	}
	return 0;
}
...
struct DirEntry * FindFreeDirEntry()
{
	...
	{
		if (RootDirectory[count].name[0] == 0) return &RootDirectory[count];
		count++;
		if (count == RootDirectoryEntries) return 0;
	}
...
{
	if (FindFile(name)) return 1;
	struct DirEntry * entry;
	if ((entry = FindFreeDirEntry()) == 0) return 1;
	int count;
	...
	if (NameToDirName(name, entry->name)) return 1;
...
long WriteFile(struct FCB * fHandle, char * buffer, long noBytes)
{
	...
	while (bytesWritten < noBytes)
	{
		...
		{
			WriteSector(ClusterToSector(fHandle->currentCluster) + fHandle->sectorInCluster - 1, fHandle->filebuf);
			if (fHandle->sectorInCluster++ > SectorsPerCluster)
			{
