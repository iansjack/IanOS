...
void ScrollScreen(long console)
{
	...
	for (row = 1; row < 25; row++)
		for (column = 0; column < 80; column++) {
			ConsoleBuffer[160 * (row - 1) + 2 * column] =
			    ConsoleBuffer[160 * row + 2 * column];
			ConsoleBuffer[160 * (row - 1) + 2 * column + 1] =
			    ConsoleBuffer[160 * row + 2 * column + 1];
		}
	for (column = 0; column < 80; column++)
	...
	if (currentBuffer == console) {
		for (row = 1; row < 25; row++)
			for (column = 0; column < 80; column++) {
				VideoBuffer[160 * (row - 1) + 2 * column] =
				    VideoBuffer[160 * row + 2 * column];
				VideoBuffer[160 * (row - 1) + 2 * column + 1] =
				    VideoBuffer[160 * row + 2 * column + 1];
			}
		for (column = 0; column < 80; column++)
...
void PrintChar(unsigned char c, long console)
{
	...
	default:
		currCons->ConsoleBuffer[160 * currCons->row +
					2 * currCons->column] = c;
		if (currCons->colour == REVERSE)
			currCons->ConsoleBuffer[160 * currCons->row +
			    ...
			    0x70;
		if (currentBuffer == console) {
			VideoBuffer[160 * currCons->row +
				    2 * currCons->column] = c;
			if (currCons->colour == REVERSE)
				VideoBuffer[160 * currCons->row +
					    2 * currCons->column + 1] = 0x70;
		}
		currCons->column++;
