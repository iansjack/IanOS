...
int
DoStat(FD fileDescriptor, struct FileInfo *info) // *** No - this should take a filename!!!
{
  int retval = -1;
...
int
DoFStat(FD fileDescriptor, struct FileInfo *info)
{
  int retval = -1;
...
long
DoRead(FD fileDescriptor, char *buffer, long noBytes)
{
  if (!noBytes)
    return 0;
...
long
DoWrite(FD fileDescriptor, char *buffer, long noBytes)
{
  if (!noBytes)
    return 0;
...
int
Do_Seek(FD fileDescriptor, int offset, int whence)
{
  long retval = -1;
...
int
kprintf(int row, int column, unsigned char *s, ...)
{
  va_list ap;
  va_start(ap, s);
